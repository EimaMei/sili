#define SI_IMPLEMENTATION 1
#include <sili.h>
#include <tests/test.h>


void test_bit(void);
void test_overflow(void);

int main(void) {
	test_bit();
	test_overflow();
}


#define bit__test(func, check, bit) { \
	u64 tests[] = {UINT64_MAX, SI_BIT(63), 1, 0xAAAAAAA, 0x22041}; \
	for_range (j, 0, si_countof(tests)) { \
		for_range (k, 0, 2) { \
			u64 test_in = tests[j]; \
			for_range (i, 0, 65) { \
				isize result = func(u64, test_in); \
				isize correct = check(test_in, bit); \
				if (result != correct) { \
					si_printfLn("%L: %s: %.64#b: result = %i, correct = %i (j = %i, k = %i)", SI_CALLER_LOC, SI_STR(#func), test_in, result, correct, j, k); \
					SI_PANIC(); \
				} \
				if (k == 0) { \
					test_in >>= 1; \
				} \
				else { \
					test_in <<= 1; \
				} \
			} \
		} \
	} \
}

siIntern
isize bits_count(u64 n, u64 bit) {
    isize count = 0;
    for_range (i, 0, 64) {
        count += (n & 1) == bit;
        n >>= 1;
    }
    return count;
}

siIntern
isize bits_countLeading(u64 n, u64 bit) {
	isize count = 0;
    for (isize i = 63; i >= 0; i -= 1) {
        u64 current_bit = (n & SI_BIT(i)) != 0;
        if (current_bit != bit) break;
        count += 1;
    }
    return count;
}

siIntern
isize bits_countTrailing(u64 n, u64 bit) {
    isize count = 0;
	for_range (i, 0, 64) {
        u64 current_bit = (n & SI_BIT(i)) != 0;
        if (current_bit != bit) break;
        count += 1;
    }
    return count;
}

#define BIT_TEST(function) \
	bit__test(si_##function##Ones, bits_##function, 1) \
	bit__test(si_##function##Zeros, bits_##function, 0) \
	SUCCEEDED()


void test_bit(void) {
	TEST_START();

	BIT_TEST(count);
	BIT_TEST(countLeading);
	BIT_TEST(countTrailing);

	TEST_COMPLETE();
}


#if SI_COMPILER_GCC || SI_COMPILER_CLANG
#define test(type, min, max) \
	do { \
		for (i64 a = min; a < max - max / 16; a += max / 16) { \
			type limit = si_max(type, 1, max / UINT16_MAX / 64); \
			for (i64 b = min; b < max - limit; b += limit) { \
				type res[2]; \
				ASSERT_FMT(si_checkAdd(type, (type)a, (type)b, &res[0]) == __builtin_add_overflow((type)a, (type)b, &res[1]), SI_STR(#type ": %lli %lli %lli %lli"), a, b, res[0], res[1]); \
				ASSERT(res[0] == res[1]); \
				\
				ASSERT_FMT(si_checkSub(type, (type)a, (type)b, &res[0]) == __builtin_sub_overflow((type)a, (type)b, &res[1]), SI_STR(#type ": %lli %lli %lli %lli"), a, b, res[0], res[1]); \
				ASSERT(res[0] == res[1]); \
				\
				ASSERT_FMT(si_checkMul(type, (type)a, (type)b, &res[0]) == __builtin_mul_overflow((type)a, (type)b, &res[1]), SI_STR(#type ": %lli %lli %lli %lli"), a, b, res[0], res[1]); \
				ASSERT(res[0] == res[1]); \
			} \
		} \
		SUCCEEDED(); \
	} while (0)

	#pragma GCC diagnostic push
	#pragma GCC diagnostic ignored "-Wsign-compare"
	#pragma GCC diagnostic ignored "-Wsign-conversion"
#else
	#define test(type, min, max)
#endif

void test_overflow(void) {
	TEST_START();

	test(u8, 0, UINT8_MAX);
	test(u16, 0, UINT16_MAX);
	test(u32, 0, UINT32_MAX);
	test(u64, 0, UINT64_MAX);
	test(usize, 0, USIZE_MAX);

	test(i8, INT8_MIN, INT8_MAX);
	test(i16, INT16_MIN, INT16_MAX);
	test(i32, INT32_MIN, INT32_MAX);
	test(i64, INT64_MIN, INT64_MAX);
	test(isize, ISIZE_MIN, ISIZE_MAX);
	
	TEST_COMPLETE();
}
